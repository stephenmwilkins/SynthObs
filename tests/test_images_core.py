...
if do_test1:
    ...
    for smoothing in [('convolved_gaussian', (1.5/h)/(1.+redshift)), ('adaptive', 8.)]:
        ...
        img = SynthObs.Morph.images_NEW.core(test.X, test.Y, test.Masses, resolution = resolution, ndim = ndim, smoothing = smoothing, verbose = True)

        fig, axes = plt.subplots(1, 2, figsize = (5,5))
        fig.subplots_adjust(left=0.0, bottom=0.0, right=1.0, top=1.0, wspace=0.0, hspace=0.0)
    
        axes[0].imshow(img.hist, interpolation = 'nearest')
        axes[1].imshow(img.data, interpolation = 'nearest')
